...
E-mails serão enviados por `POST /users/{MICROSOFT_NOTIFICATION_SENDER}/sendMail`. O caminho recomendado é Exchange Online Application RBAC, atribuindo `Application Mail.Send` apenas à mailbox/grupo remetente, sem consentimento global Entra `Mail.Send`; a alternativa de compatibilidade é `Mail.Send` no Entra restrito por Application Access Policy. Os caminhos são mutuamente exclusivos porque grants são aditivos; `User.Read.All` permanece consentido para a resolução de destinatários.
